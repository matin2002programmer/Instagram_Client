...
class InstagramClient:
    ...
    def _handle_checkpoint(self, checkpoint_url: str) -> bool:
        ...
        if step_name == "select_verify_method":
            ...
            resp = self._request("POST", url, headers=headers, data={"choice": choice})
    ...
    def _extract_shortcode(self, url: str) -> str:
        ...
        match = re.match(r"https?://www\.instagram\.com/(p|reels)/([^/]+)/?", url.strip().rstrip(','))
        if match:
...
if __name__ == "__main__":
    cli = InstagramClient()
    # cli.login("matindevilish_boy", "Matin.M&Somayeh.D81=Love")
    # cli.download_post("https://www.instagram.com/reels/DNv-rrY4srJ/")
    # cli.download_story("https://www.instagram.com/stories/psg/3705717631640806134/")
    # cli.download_stories("fcbarcelona")
    # cli.comment_on_first_post("jhefrymawu7", "Nice!")
    # cli.comment_on_post("https://www.instagram.com/p/DNwRLtW2gLy/?utm_source=ig_web_copy_link", "shame on israel")
    # cli.upload_reel(video_path="./downloads/😐_#militaryaviation_#aviation_#military_DKKHNQcMixM.mp4",
    #                caption="moon :)")
